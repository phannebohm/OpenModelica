  ...
  for subScript in inSubscriptLst loop
    outCref := match(subScript)
      ...
      case DAE.INDEX(exp = e) equation
        // phannebohm: this is perhaps wrong for FOR_EQUATIONs? Check if exp is iterator
        _ = match e
          local
            DAE.ComponentRef crr;
          case DAE.CREF(componentRef = crr) guard substring(crefStr(crr), 1, 1) <> "$"
            then "";
          else "";
        end match;
